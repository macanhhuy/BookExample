...
(function() {
	...
	describe('The person can sign in if He registered before with his email', function() {
		// Initialize global variables
		var AuthenticationController,
			scope,
			$httpBackend,
		...
		it('can login with a correct user and password', function() {
			// Test expected GET request
			$httpBackend.when('POST', '/auth/signin').respond(200, 'Felix');

			scope.signin();
			$httpBackend.flush();

			// Test scope value
			expect(scope.authentication.user).toEqual('Felix');
			expect($location.url()).toEqual('/');
		});
